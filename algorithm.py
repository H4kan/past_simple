...
def computeDistFromAlignments(plText, enText, probs, bestAlignments):
    ...
    distCounter = {sentLen: {enIdx: {plIdx: 0 for plIdx in range(0, constraints.MAX_PL_LEN)} for enIdx in range(0, sentLen)} for sentLen in range(1, constraints.MAX_EN_LEN + 1)};
    generalCounter = {sentLen: {enIdx: 0 for enIdx in range(0, sentLen)} for sentLen in range(1, constraints.MAX_EN_LEN + 1)};
        ...
        for enIdx in range(0, sentLen):
            for plIdx in range(0, constraints.MAX_PL_LEN):
                if generalCounter[sentLen][enIdx] > 0:
                    probs.dist[sentLen][enIdx][plIdx] = distCounter[sentLen][enIdx][plIdx] / generalCounter[sentLen][enIdx];
                else:
                    probs.dist[sentLen][enIdx][plIdx] = 0;

def computeTransFromAlignments(plText, enText, probs, bestAlignments):

    transCounter = {pl: {en: 0 for en in (probs.allEnWords + ["\0"])} for pl in (probs.allPlWords + ["\0"])};
    ...
    for plWord in (probs.allPlWords + ["\0"]):
        for enWord in (probs.allEnWords + ["\0"]):
            probs.trans[plWord][enWord] = transCounter[plWord][enWord] / generalCounter[plWord];
